...
class PER():
    ...
    def plot(self, eva100_reward, mean100_reward, epsilon):
        fig = plt.gcf()
    ...
    def interaction_step(self, state, env:MarioGame):
    ...
    def train(self, env:MarioGame, gamma, 
        ...
        for episode in range(1, max_episodes + 1):
            ...
            infos = {"episode" : episode, "evaluation" : False, "epsilon" :  self.training_strategy.epsilon}
            state, is_terminal = env.reset(options=infos), False
            ...
            self.epsilons.append(self.training_strategy.epsilon)
    ...
    def evaluate(self, eval_policy_model, eval_env:MarioGame, n_episodes=1):
        rs = []
        for _ in range(n_episodes):
            infos = {"episode" : 0, "evaluation" : True, "epsilon" :  self.training_strategy.epsilon}
            s, d = eval_env.reset(options= infos), False
            rs.append(0)
            for _ in count():
