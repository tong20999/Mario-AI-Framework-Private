...
public class RewardSystem {
    ...
    static float winRewardMultiplier = 2;
    static float mileStoneReward = 0.0f;
    static float jumpOverPitReward = 0f;
    static float killReward = 2;
    static float bumpReward = 1;
    static float coinReward = 1;
    static float fireworkReward = 1;
    static float mushroomReward = 1;
    static float lifeMushroomReward = 1;
    static float loseReward = -0f;
    static float loseTimeoutReward = -0f;
    static float hurtReward = -0f;
    static float hitWallReward = -0.0f;
    static float fallPitReward = -0f;
    static float timePenaltyRewardCoefficient = 0;
    static float jumpSpamPenalty = -0.0f;
    static float clipReward = 25;

    public static float getReward(MarioWorld world, ArrayList<MarioEvent> miniStepEvents, boolean[] action) {
        float reward = 0.0f;
        //reward += timePenalty();
        //reward += mileStoneReward();

//        if(action[4] && !(world.mario.jumpTime > 0)){
//            reward += jumpSpamPenalty;
//        }
        ...
        if (world.gameStatus == GameStatus.WIN) {
            ...
            float total = (world.kill * killReward) + (world.bumpBlock * bumpReward) + (world.collectCoin * coinReward);
